...
def load_user_database():
    ...
    return {
        "admin_owner": {
            "password": hashlib.sha256("OwnerPass999".encode()).hexdigest(),
            "role": "OWNER"
        }
    }
...
if "global_chat_history" not in st.session_state:
    st.session_state["global_chat_history"] = [{"user": "System", "tag": "SYSTEM", "time": datetime.datetime.now().strftime("%H:%M"), "content": "Welcome back. Persistent student accounts are now active."}]
    ...
    with tab2:
        ...
        if st.button("Create Permanent Account", use_container_width=True):
            ...
            if not name_clean or " " in name_clean:
                st.error("❌ Nickname cannot have spaces.")
            elif name_clean in user_db:
                st.error("❌ That username is already taken.")
            elif invite_code != "School123":
                st.error("❌ Incorrect school invite code.")
            elif len(reg_pass) < 4:
                st.error("❌ Password must be at least 4 characters long.")
            else:
                user_db[name_clean] = {"password": hash_pass(reg_pass), "role": "STUDENT"}
                save_user_database(user_db)
                st.success("🎉 Account created! Switch to the 'Log In' tab to enter.")
    st.stop()
...
if raw_input := st.chat_input("Type a message or owner command..."):
    ...
    if my_name in st.session_state["timeout_users"]:
        ...
        if current_time < expiry:
            ...
            st.error(f"You are timed out. Remaining time: ~{remaining_minutes} minute(s).")
            st.stop()
        else:
            ...
            if command_text == "/clear":
                ...
                try:
                    ...
                    if target_user != my_name:
                        ...
                        st.session_state["global_chat_history"].append({"user": "System", "tag": "SYSTEM", "time": datetime.datetime.now().strftime("%H:%M"), "content": f"User '{target_user}' has been placed in timeout for {duration_minutes} minute(s)."})
                        st.rerun()
                except:
                    st.error("Error! Use syntax: `/timeout [Username] [minutes]`")
                    st.stop()
            elif command_text.startswith("/alert "):
                alert_msg = command_text.replace("/alert ", "").strip()
                st.session_state["global_chat_history"].append({"user": my_name, "tag": "OWNER ALERT", "time": datetime.datetime.now().strftime("%H:%M"), "content": alert_msg.upper()})
                st.rerun()
        else:
            user_tag = "OWNER" if st.session_state["is_owner"] else "STUDENT"
            st.session_state["global_chat_history"].append({
                "user": my_name, "tag": user_tag,
